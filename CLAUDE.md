...
| `import_objectives.py`     | Imports raw objectives into a course's pool, **interning by text** (find-or-create by exact text, so runs are idempotent and identical texts collapse onto one objective; uuids are generated). Two auto-detected inputs: plain text (one objective per line, pool only) or a TSV with an `objective` column and optional `node_id` (a coverage edge into the course's reference, or `--hierarchy <slug>`; `node_id` implies its ancestors). `--replace` re-seeds the course's pool/coverage; warns on node_ids absent from `nodes` |
