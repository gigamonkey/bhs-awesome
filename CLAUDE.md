...
| `dedup.py`                 | Dedup pre-filter library: clusters candidate-duplicate objectives by character-k-gram Jaccard (or LCS) similarity, prioritizing pairs that share a coverage node. Surfaces candidates only — semantic judgment and merging happen in the app. Used by `dedup_candidates.py` and `lesson-planning/app.py` |
| `dedup_candidates.py`      | CLI over `dedup.py`: prints candidate duplicate-objective clusters for a course from the lesson-planning db. Options: `--course`, `--method jaccard\|lcs`, `--threshold` |
