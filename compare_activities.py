...
stable UUIDs, then writes <outdir>/paired.tsv with four columns:

  type  uuid_a  uuid_b  similarity

For each activity type, the side with fewer activities drives the matching: each
of its activities is paired with its best-scoring match from the other side.
Leftover activities on the larger side (those not claimed by any match) are
emitted with '-' in the missing UUID column and 0.0 as the score.
"""
...
def match_type(atype, acts_a, acts_b, paired_f):
    ...
    if not unmatched_a or not unmatched_b:
        # Emit any leftovers on either side as unmatched
    ...
    if a_is_smaller:
        ...
        def make_row(uid_small, uid_large, sim): return f"{atype}\t{uid_small}\t{uid_large}\t{sim}\n"
    else:
        smaller, larger = unmatched_b, unmatched_a
        def make_row(uid_small, uid_large, sim): return f"{atype}\t{uid_large}\t{uid_small}\t{sim}\n"
...
def compare_activities(root_a, root_b, outdir):
    ...
    with open(paired_path, "w", buffering=1) as paired_f:
        # Types only on one side — all unmatched
        ...
        for atype in sorted(types_a & types_b):
            ...
            match_type(atype, acts_a, acts_b, paired_f)
...
if __name__ == "__main__":
    ...
    compare_activities(args.root_a, args.root_b, args.outdir)
